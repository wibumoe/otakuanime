...
const Detail = async function (id) {
    ...
    $(".venser").children().each(function(){
        const item = $(this);
        let cover = item.find(".fotoanime > img").attr("src");
        let endpoint = item.find("$id").attr("href");
        ...
        if(endpoint && cover && synopsis && rating && title && title_jp && rating && producer && status && total_episode && duration && release_date && studio && genre){
        ...
        if(!synopsis.length <1){
            detail.push({endpoint:endpoint,cover:cover,title:title,title_jp:title_jp,rating:rating,producer:producer,status:status,total_episode:total_episode,duration:duration,release_date:release_date,genre:genre,synopsis:synopsis});
        }
    });
    const episode = [];
